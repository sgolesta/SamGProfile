...
public class TfLOverlayManager implements MapEventsReceiver {
    ...
    public TfLOverlayManager(MapDisplayFragment mapFragment, MapView mapView, GpsMyLocationProvider locnProvider) {
        ...
        polyline = new Polyline(mapView.getContext());
        lines = new HashSet<>();
    ...
    public void updateMarkerOfNearest(Station nearest) {
        ...
        List<Marker> stnMarkers = stnClusterer.getItems();

        for (Marker marker: stnMarkers) {
            marker.setIcon(stnIconDrawable);
        }

        if (nearest != null) {
            for (Marker marker : stnMarkers) {
                if (marker.getRelatedObject().equals(nearest)) {
    ...
    private void markStations() {
        ...
        for (Station station: stnManager) {
            ...
            stnMarker.setTitle(station.getName());
            stnMarker.setAnchor(Marker.ANCHOR_CENTER, Marker.ANCHOR_CENTER);
            stnMarker.setRelatedObject(station);
